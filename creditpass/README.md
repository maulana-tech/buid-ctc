...
| `zk/HonkVerifier.sol` | UltraHonk verifier generated by Barretenberg from `circuits/credit_threshold` — the private borrow path. Built by the `zk` Foundry profile |
| `zk/PoseidonT3` (poseidon-solidity) | Poseidon over BN254 with circomlib parameters, deployed once and called by the registry to keep its Merkle tree |
...
The Noir circuit (`circuits/credit_threshold`, 6,584 constraints) proves: I know `secret` whose
commitment sits in a leaf under `root` together with an address and its score, `nullifier =
H(secret, 1)`, and `score ≥ T + 300 × defaults`. Public inputs are exactly `root`, `T`, `defaults`,
`nullifier`; the contract supplies `defaults` from its own storage so the prover cannot understate
it, and accepts any of the last 32 roots so a proof is not invalidated by an unrelated update.

- The borrowing wallet is public; the scored address and the exact score are not.
- Limit = the tier `T` falls in. Prove a lower band than you hold and you borrow less — selective
  disclosure is the feature.
- One open loan per identity (`nullifierHasLoan`). Repay releases it.
- A default on a private loan is recorded against the nullifier, never the address. Every later
  private proof must subtract 300 per default; the public passport stays clean. The identity carries
  the mark, which is the whole point of a nullifier that is deterministic per secret.
- Proving happens in the browser (noir_js + bb.js, WASM). About 30s single-threaded in a tab, 3s
  multi-threaded in Node. The secret never leaves the browser.

`test/PrivateCredit.t.sol` runs a real UltraHonk proof through the real verifier. Its first
assertion is that the contract's Poseidon tree reaches the same root JavaScript computed
(`script/make-zk-fixture.ts`), so a hashing or layout mismatch fails there, not as an opaque
`InvalidProof`. `npm run zk:selftest` in `web/` proves the same statement with the browser libraries.
...
- **Public and private credit are two channels on one identity.** An address can draw its public
  limit *and* its private limit at once, because the contract cannot link the two. Closing that means
  making the private path the only path. Left open on testnet so both can be demonstrated.
- **Every score change rewrites a tree path.** ~450k gas per proved event on top of the ASC's own
  cost. Fine at hackathon scale; LeanIMT with caller-supplied siblings if writes need to be cheap.
- **Honk verifier at optimizer_runs = 1.** The zero-knowledge flavour is 24,061 bytes — 515 under
  EIP-170 — only at that setting. Verify costs ~2M gas.
